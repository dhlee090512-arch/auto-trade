...
# ==========================================
# 1. 전역 설정 및 스윙 파라미터 (적정 밸런스)
# ==========================================
PAPER_TRADING = True             # 🧪 True: 모의투자 / False: 빗썸 실전매매
MIN_CONFIDENCE_SCORE = 65        # 🎯 스윙 진입 기본 최소 신뢰도
...
# 📈 스윙 손익비 및 트레일링 파라미터
BREAK_EVEN_TRIGGER_PCT = 2.0     # 🛡️ 본절 방어선 발동 기준 (+2.0% 수익 시)
TRAILING_START_PCT = 3.5         # 🚀 트레일링 스탑 활성화 기준 (+3.5% 돌파 시)
TRAILING_GAP_PCT = 1.0           # 🚀 고점 대비 하락 허용폭 (-1.0% 반락 시 이익 보존 청산)
...
SESSION_START_TIME = datetime.now(KST)
...
# ==========================================
# 2. 텔레그램 및 KST 타임존 안전 유틸리티
# ==========================================
def get_kst_now():
    return datetime.now(KST)

def parse_kst_iso(iso_str: str) -> datetime:
    """어떤 형태의 ISO 문자열도 안전하게 KST datetime 객체로 변환"""
    if not iso_str:
        return get_kst_now()
    try:
...
def send_telegram_msg(msg: str):
    ...
    url = "".join(["http", "s://api.telegram.org/bot", clean_token, "/sendMessage"])
...
def format_portfolio_status_msg(active_positions, closed_trades):
    ...
    if not recent_10:
        ...
        for idx, t in enumerate(recent_10, 1):
            ...
            dt_obj = parse_kst_iso(exit_time_str)
    ...
    for t in closed_trades:
        exit_dt = parse_kst_iso(t.get('exit_time', ''))
...
def sync_file_to_github(file_path, content_data):
    if not GH_TOKEN:
        return
    url = "".join(["http", "s://api.github.com/repos/", GITHUB_REPOSITORY, "/contents/", file_path])
...
def get_real_krw_balance():
    if BITHUMB_API_KEY and BITHUMB_SECRET_KEY:
        try:
            url = "".join(["http", "s://[api.bithumb.com/v1/accounts](https://api.bithumb.com/v1/accounts)"])
...
def execute_real_market_order(coin_code: str, side: str, amount_or_units: float):
    ...
    try:
        ...
        url = "".join(["http", "s://[api.bithumb.com/v1/orders](https://api.bithumb.com/v1/orders)"])
...
def get_current_price(coin_code: str) -> float:
    try:
        ...
        url = "".join(["http", "s://[api.bithumb.com/public/ticker/](https://api.bithumb.com/public/ticker/)", clean_code, "_KRW"])
...
def get_candles(coin_code, interval="1h", limit=50):
    try:
        clean_code = coin_code.replace("KRW-", "").replace("/KRW", "").replace("[", "").replace("]", "").strip()
        url = "".join(["http", "s://[api.bithumb.com/public/candlestick/](https://api.bithumb.com/public/candlestick/)", clean_code, "_KRW/", interval])
...
def calculate_swing_quant_features(candles_1h, candles_15m):
    ...
    return {
        ...
        "ema20_1h": ema20_1h,
        "ema60_1h": ema60_1h,
        "is_1h_bullish_aligned": (c1h[-1] >= ema20_1h * 0.995 and ema20_1h >= ema60_1h * 0.990),
...
def calculate_swing_score(q, btc_ret_4h) -> int:
    ...
    if alt_ret > btc_ret_4h + 0.5: score += 25
    elif alt_ret > btc_ret_4h: score += 15

    rsi15 = q.get("rsi_15m", 50)
    if 38 <= rsi15 <= 58: score += 20
    elif 34 <= rsi15 < 38 or 58 < rsi15 <= 64: score += 10
...
def call_ai_api(system_instruction, user_prompt):
    providers = []
    if GEMINI_API_KEY:
        providers.append({"name": "Gemini", "key": GEMINI_API_KEY, "base_url": "".join(["http", "s://[generativelanguage.googleapis.com/v1beta/openai/](https://generativelanguage.googleapis.com/v1beta/openai/)"]), "model": "gemini-3.5-flash-lite"})
    if SAMBANOVA_API_KEY:
        providers.append({"name": "SambaNova", "key": SAMBANOVA_API_KEY, "base_url": "".join(["http", "s://api.sambanova.ai/v1"]), "model": "Meta-Llama-3.3-70B-Instruct"})
    if GROQ_API_KEY3 or GROQ_API_KEY2:
        providers.append({"name": "Groq", "key": GROQ_API_KEY3 or GROQ_API_KEY2, "base_url": "".join(["http", "s://[api.groq.com/openai/v1](https://api.groq.com/openai/v1)"]), "model": "llama-3.3-70b-specdec"})
...
• 조치 : 신규 알트코인 스윙 탐색 일시정지 (현금 100% 방어)"""
    ...
    url = "".join(["http", "s://[api.bithumb.com/public/ticker/ALL_KRW](https://api.bithumb.com/public/ticker/ALL_KRW)"])
    try:
    ...
    for sym, price, change, _ in sorted_list:
        ...
        if q.get("rsi_1h", 50) > 70.0: continue
    ...
    sys_2 = (
        ...
        "2. Swing Targets: take_profit_pct: +4.5% to +8.0% (trailing run applies), stop_loss_pct: -1.8% to -2.3%.\n"
        "3. Output valid JSON ONLY."
    )
    user_2 = f"15m Series Data:\n{json.dumps(cand_15m_data, ensure_ascii=False)}\n\nSchema: {{\"selected_symbol\": \"BTC/KRW\", \"confidence_score\": 75, \"entry_discount_pct\": {bandit_discount}, \"stop_loss_pct\": -2.0, \"take_profit_pct\": 5.5, \"detailed_reason\": \"진입근거\"}}"
    ...
    sl_pct = max(min(float(res_2.get("stop_loss_pct", -2.0)), -1.8), -2.4)
    tp_pct = min(max(float(res_2.get("take_profit_pct", 5.5)), 4.0), 10.0)
...
=================================
⚡ 규칙: 2시간 미체결 취소 / +2.0% 본절 이동 / +3.5% 트레일링 가동"""
...
💰 투입 금액 : {plan['buy_amount_krw']:,} KRW"""
            ...
            # [2] 보유 포지션 감시 (타임존 안전 계산 및 만기 자동 정리)
            for coin_code, pos in list(active_positions.items()):
                curr_p = get_current_price(coin_code)
                entry_p = pos["entry_price"]
                entry_time = parse_kst_iso(pos.get("entry_time", ""))

                # 현재가를 못 가져와도 18시간 만기 초과 시 진입가 기준으로 강제 정리
                if not curr_p:
                    if (now_dt - entry_time) >= timedelta(hours=MAX_HOLD_HOURS):
                        curr_p = entry_p
                    else:
                        continue
                ...
                if not pos.get("break_even_triggered", False) and curr_profit_pct >= BREAK_EVEN_TRIGGER_PCT:
                    pos["stop_loss"] = max(pos["stop_loss"], entry_p * 1.002)
                    pos["break_even_triggered"] = True
                    logging.info(f"🛡️ [{pos['symbol']}] +2.0% 도달로 본절 방어선(+0.2%) 세팅")

                status = "HOLDING"
                exit_reason = ""

                # ① 무제한 트레일링 스탑 (+3.5% 돌파 후 고점 대비 1.0% 반락 시)
                ...
                # ④ 18시간 횡보 박스권 청산
                elif (now_dt - entry_time) >= timedelta(hours=MAX_HOLD_HOURS):
                    status = "CLOSED_TIME_EXIT"
                    exit_reason = f"⏰ {MAX_HOLD_HOURS}시간 최대 보유 시간 초과로 정리"
...
def telegram_listener_thread():
    ...
    clean_token = token.split("/")[-1].replace("[", "").replace("]", "").strip()
    base_url = "".join(["http", "s://api.telegram.org/bot", clean_token, "/getUpdates"])
