...
MAX_HOLDING_COINS = 3            # 최대 동시 보유 종목 수
MIN_BUY_KRW = 10000              # 최소 매수 금액 (원)
BUY_RATIO = 0.25                 # 1회 투입 비중 (25%)
ENTRY_TIMEOUT_MINUTES = 120      # 15분봉 스윙 눌림목 대기 시간 (2시간)

# 스윙 손익비 및 트레일링 파라미터
BREAK_EVEN_TRIGGER_PCT = 2.0     # 본절 방어선 발동 기준 (+2.0%)
TRAILING_START_PCT = 3.5         # 트레일링 스탑 활성화 기준 (+3.5%)
TRAILING_GAP_PCT = 1.0           # 고점 대비 하락 허용폭 (-1.0%)
...
# ==========================================
# 1. 텔레그램 및 KST 타임존 안전 유틸리티
# ==========================================
def get_kst_now():
    return datetime.now(KST)

def parse_kst_iso(iso_str: str) -> datetime:
    """타임존 유무에 상관없이 안전하게 KST datetime 객체로 변환"""
    if not iso_str:
        return get_kst_now()
    try:
...
• 조치 : 신규 알트코인 스윙 탐색 일시정지 (현금 100% 방어)"""
    ...
    for sym, price, change, _ in sorted_list:
        ...
        if q.get("rsi_1h", 50) > 68.0: continue
