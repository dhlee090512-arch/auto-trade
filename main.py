...
def call_groq_api(system_instruction, user_prompt):
    ...
    try:
        ...
        last_error = str(e)
        print(f"⚠️ GROQ_API_KEY3 호출 실패: {last_error}")
        send_telegram_msg(f"⚠️ <b>[Groq API 에러]</b>: <code>{last_error}</code>")
        return None
